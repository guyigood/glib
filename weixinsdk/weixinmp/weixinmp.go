...
func (this *Weixinmp) CreateCustomMenu(btn *[]Button) error {
	...
	for i := 0; i < retryNum; i++ {
		...
		//fmt.Println(buf)
		//if _, err := post(url+token, "application/json;charset=utf-8", buf); err != nil {
		if _, err := Http_post_json(url+token, string(jsonData)); err != nil {
			if i < retryNum-1 {
				continue
			}
			//fmt.Println("create menu error", err,m_re)
			return err
		}
		break // success
	}
