...
func NewMySql_Server_DB(action string) (*Mysqlcon) {
	...
	data := common.Getini("conf/app.ini", action, map[string]string{"db_user": "root", "db_password": "",
		"db_host": "127.0.0.1", "db_port": "3306", "db_name": "nqht", "db_maxpool": "20", "db_minpool": "5", "db_perfix": "", "slavedb": "","db_type":""})
    if(data["db_type"]!=""){
    	return nil
	}
...
func (this *Mysqlcon) Max(fd string) (float64) {
	var result float64
	sqltext := this.set_sql(1)
	sqltext = strings.Replace(sqltext, "count(*)", "IFNULL(max("+fd+"),1)", -1)
	sqlwhere, param := this.Build_where()
	sqltext += sqlwhere
	this.LastSqltext = sqltext
	sqldbcon := this.Get_read_dbcon()
...
func (this *Mysqlcon) IMin(fd string) (int64) {
	var result int64
	sqltext := this.set_sql(1)
	sqltext = strings.Replace(sqltext, "count(*)", "mIn("+fd+")", -1)
	sqlwhere, param := this.Build_where()
	sqltext += sqlwhere
	this.LastSqltext = sqltext
	sqldbcon := this.Get_read_dbcon()
